...
Steam-mediated game launching is live (v0.4.0 "Launch Day"). Press **`l`** on a library row (or **`Enter`/`l`** in the game-detail view) to launch the highlighted game; **`L`** opens a recently-played **quick-launch overlay** (`views/quick_launch.rs` + `handlers/quick_launch.rs`, an `ActiveBlock::QuickLaunch` modal mirroring the help/error overlays). Launching is a **local system action**, not a Steam-protocol RPC: the new `IoEvent::LaunchGame(appid)` is handled in `network.rs` via `tokio::task::spawn_blocking`, which calls `vapour-core::launcher::launch`. The launcher resolves the Steam executable (Windows registry via `reg.exe` then `%ProgramFiles(x86)%\Steam\steam.exe`; Linux `PATH`/`~/.steam/steam.sh`; macOS `Steam.app`) and runs `steam -applaunch <appid> [args]`, so Steam starts itself when down and presence/"playing X" is handled by the official client we delegate to — **no `vapour-protocol` changes**. Launch options come from the `[launch]` config section (`vapour-core::config::LaunchConfig::options_for` merges global `extra_args` with per-game `game_args`); a transient `App.launch_status` shows the result (and, in `dry_run` mode, the exact command — also logged via `tracing` under `target: "launch"`). The optional, off-by-default `kill_steam_on_exit` spawns a detached best-effort watcher: only if Vapour started Steam, it polls Steam's per-app running flag (Windows registry / Linux `registry.vdf`) and runs `steam -shutdown` once the game exits. No new crates were added (offline-friendly: `reg.exe` + filesystem reads; the string parsers are pure and unit-tested). **Deferred to a later release (experimental):** no-Steam direct executable launch for DRM-free games, DRM detection/gating, and external Proton/Wine-prefix wrapping (Steam applies Proton itself for Steam-mediated launches). **Known limitations:** kill-on-exit detection is best-effort (reliable on Windows via the registry; weaker on Linux, a no-op on macOS); per-game `game_args` are whitespace-split (no quoted-arg grouping); the quick-launch overlay opens from the library only.
