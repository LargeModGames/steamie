...
pub fn handle(app: &mut App, key: Key) {
    match key {
        Key::Char('j') | Key::Down => {
            app.pending_g = false;
            let len = app.wishlist.len();
            App::scroll_down(&mut app.wishlist_state, len);
        }
        Key::Char('k') | Key::Up => {
            app.pending_g = false;
            App::scroll_up(&mut app.wishlist_state);
        }
        Key::Char('g') => {
            if app.pending_g {
                app.pending_g = false;
                App::scroll_top(&mut app.wishlist_state);
            } else {
                app.pending_g = true;
            }
        }
        Key::Char('G') => {
            app.pending_g = false;
            let len = app.wishlist.len();
            App::scroll_bottom(&mut app.wishlist_state, len);
        }
        Key::Char('r') => {
            app.pending_g = false;
            app.dispatch(crate::io_event::IoEvent::LoadWishlist);
        }
